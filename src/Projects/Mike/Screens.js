...
export default function Screens() {
  ...
  return (
    <div className='row-start-2 relative overflow-hidden' id='screen-container'>
        ...
        :   dataHeb.map((e,i) => {
            return (
                <div key={i} className='h-full w-full grid grid-cols-[75%,_25%] grid-rows-[75%,_25%] absolute left-0 top-0 data-[status=moving-right]:transition-none data-[status=moving-right]:translate-x-[110%] data-[status=moving-left]:transition-none data-[status=moving-left]:-translate-x-[110%] data-[status=active]:transition-all data-[status=active]:duration-500 data-[status=before]:-translate-x-[110%] data-[status=after]:translate-x-[110%]' data-index={i} data-status={i === 0 ? 'active' : 'before'}>
